...
<template>
    <div class="relative p-4 my-4 overflow-hidden text-sm border w-52 min-h-52" @mouseover="show = true" @mouseleave="show=false" :data="{ id: product.id }">
      <Transition name="fade-slide" appear >
            <div v-if="show" class="absolute w-10 top-1 right-2">
            <button type="button" :disabled="isAlreadyInCart" @click="addToCart(product)" class="flex items-center justify-center w-full p-2 my-2 bg-white border border-gray-300 rounded-full hover:bg-gray-50" >
            ...
            <button type="button" class="flex items-center justify-center w-full p-2 bg-white border border-gray-300 rounded-full hover:bg-gray-50" >
        ...
        <Link :href="route('product.about',{id:product.id})">
          <div>
            <img :src="'/images/product/'+ product.image" class="object-cover mx-auto h-44"/>
          </div>
          <div>
          ...
          <div class="absolute px-3 py-1 font-semibold bg-yellow-400 top-1 left-1">
